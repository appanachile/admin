<x-app-layout>
    <section class="min-h-screen flex items-stretch text-white ">
        ...
        <div class="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-4 z-0" style="background-color: #161616;">
            ...
            <div class="w-full py-6 z-20">
                ...
                <div class="py-6 space-x-2 hidden">
                    ...
                    <span class="w-10 h-10 items-center justify-center inline-flex rounded-full font-bold text-lg border-2 border-white hidden">G+</span>
                    <span class="w-10 h-10 items-center justify-center inline-flex rounded-full font-bold text-lg border-2 border-white hidden">in</span>
                </div>
                <p class="text-gray-100">
                    o haz tu registro manualmente
                </p>
                <form method="POST" action="{{ route('register') }}">
                    @csrf
        
                    <div>
                        <x-jet-label for="name" value="{{ __('Nombre') }}" class="text-white text-left" />
                        <x-jet-input id="name" class="block mt-1 w-full text-gray-900" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
                    </div>
        
                    <div class="mt-4">
                        <x-jet-label for="email" value="{{ __('Email') }}" class="text-white text-left" />
                        <x-jet-input id="email" class="block mt-1 w-full text-gray-900" type="email" name="email" :value="old('email')" required />
                    </div>
        
                    <div class="mt-4">
                        <x-jet-label for="password" value="{{ __('Contraseña') }}" class="text-white text-left" />
                        <x-jet-input id="password" class="block mt-1 w-full text-gray-900" type="password" name="password" required autocomplete="new-password" />
                    </div>
        
                    <div class="mt-4">
                        <x-jet-label for="password_confirmation" value="{{ __('Confirmar Contraseña') }}" class="text-white text-left" />
                        <x-jet-input id="password_confirmation" class="block mt-1 w-full text-gray-900" type="password" name="password_confirmation" required autocomplete="new-password" />
                    </div>
